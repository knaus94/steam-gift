...
SteamGift.prototype.setCookie = function (cookie) {
	...
	let isSecure = !!cookieName.match(/(^steamMachineAuth|^steamLoginSecure|^shoppingCartGID|^beginCheckoutCart$)/);
	this._jar.setCookie(cookie, (isSecure ? "https://" : "http://") + "store.steampowered.com");
	this._jar.setCookie(cookie, (isSecure ? "https://" : "http://") + "steamcommunity.com");
};
...
SteamGift.prototype.delCookie = function (cookieName) {
	let cookiesCommunity = this._jar.getCookieString("https://steamcommunity.com").split(';');
	let cookiesStore = this._jar.getCookieString("https://store.steampowered.com").split(';');
	...
	for (let cookie of cookiesCommunity) {
		let match = cookie.trim().match(/([^=]+)=(.+)/);
		if (match != null && match[1] != cookieName) {
			this.setCookie(match[1] + "=" + match[2])
		}
	}
	for (let cookie of cookiesStore) {
		let match = cookie.trim().match(/([^=]+)=(.+)/);
		if (match != null && match[1] != cookieName) {
			this.setCookie(match[1] + "=" + match[2])
		}
